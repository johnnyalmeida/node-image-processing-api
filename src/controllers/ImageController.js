...
class ImageController {
  ...
  processImage(data, res) {
    try {
      ...
      const file = fs.createWriteStream(`${this.config.base_path}/tmp/images/${key}`);
      const params = {
      ...
      this.s3.getObject(params).createReadStream()
        ...
        .pipe(file)
        .on('finish', () => {
          const filePath = `${this.config.base_path}/tmp/images/${key}`;
          ...
          .then((image) => {
            ...
            image
              ...
              .quality(90)
              .write(`${this.config.base_path}/tmp/images/processed/${key}`, () => {
                ...
                thumb
                  ...
                  .scaleToFit(200, 200)
                  .quality(75)
                  .write(`${this.config.base_path}/tmp/images/thumbs/${key}`, () => {
                    console.log('write thumb');
                    this.moveThumbToS3(key, `${this.config.base_path}/tmp/images/thumbs/${key}`);
                  });
              });
          }).catch((err) => {
            console.log(err);
